...
```php
为什么说套路呢，不然发现这个面试官所有准备的题都是有针对性的，后面他就指出我的基础不够扎实，并举例说一个基础扎实的人和一个基础不扎实
的人做同一个东西，基础扎实的可能很快就会做完且不会出错，而不扎实的老是需要google且做出的东西会出错，也不知道哪里错了，因为是拿来主义
并没有转为自己的东西。然后指出我没有系统学习过php，因为php容易入门，但是学会它需要花很大功夫。最后说我的规划还不错，是加分项，哈哈哈。
```
